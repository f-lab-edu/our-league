...
public class TeamController {
    ...
    @GetMapping
    @Operation(summary = "팀 목록 조회 - 지도형")
    @ApiResponse(responseCode = "200", description = "사용자 위치 주변에 있는 팀 목록을 반환한다.")
    public List<TeamMapResponse> teamListForMap(TeamSearchDTO searchDTO) {
        return teamService.getTeamListForMap(searchDTO);
    }

    @GetMapping("/{sportsId}")
    @Operation(summary = "팀 목록 조회")
    @ApiResponse(responseCode = "200", description = "검색 조건에 부합하는 팀 목록을 반환한다")
    public List<TeamMapResponse> teamList(@PathVariable Long sportsId, TeamSearchDTO searchDTO) {
    ...
    public TeamMapResponse modify(@PathVariable Long teamId, @RequestBody TeamModifyRequest request, @AuthenticationPrincipal UserDTO userDTO) {
